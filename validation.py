...
def main():
    ...
    last_pred = -1
    last_label = -1
    pbar = tqdm(range(0, len(List)))
    for idx in pbar:

        if len(timeList)>0:
            last_latency = timeList[-1]
        else:
            last_latency = -1
        last_filenm = List[idx].split(',')[1].strip()
        pbar.set_description("%s -> 1 Inference started - [latency|label|pred: %f|%s|%s]"  % (last_filenm, last_latency, last_label, last_pred))
        ...
        for i in range(0, testX.shape[0]):
            ...
            while ser.in_waiting < 5:
                pass
                time.sleep(0.01)
            pbar.set_description("%s -> 2 Inference started - [latency|label|pred: %f|%s|%s]"  % (last_filenm, last_latency, last_label, last_pred))
            ...
            if recv.strip() == 'begin':
                ...
                pbar.set_description("%s -> 3 Inference started - [latency|label|pred: %f|%s|%s]"  % (last_filenm, last_latency, last_label, last_pred))    
                time.sleep(0.01)
                recv = ser.read(size=ser.in_waiting).decode(encoding='utf8')
                ser.reset_input_buffer()
                
                # while 'ok' not in recv:
                #     print(recv.strip())
                #     while ser.in_waiting < 2:
                #         pass
                #     time.sleep(0.01)
                ...
                pbar.set_description("%s -> 4 Inference started - [latency|label|pred: %f|%s|%s]"  % (last_filenm, last_latency, last_label, last_pred))
                ...
                pbar.set_description("%s Inference Latency : %f" % (List[idx].split(',')[1].strip(), timeList[-1]))
    ofp.close()
